...
class SWAG implements IPreAkiLoadMod, IPostDBLoadMod {
  ...
  static ConfigureBossWave(boss: BossLocationSpawn, globalmap: LocationName): BossLocationSpawn {
    ...
    const getDifficulty = (configDifficulty: string, bossDifficulty?: string) => {
        const difficultyLookup = bossDifficulty || diffProper[configDifficulty.toLowerCase()];
        return difficultyLookup === "random" ? getRandomDifficulty() : difficultyLookup;
    };

    let difficulty = getDifficulty(config.BossDifficulty, boss.BossDifficult);
    let escort_difficulty = getDifficulty(config.BossEscortDifficulty, boss.BossEscortDifficult);
    let bossName = roleCase[boss.BossName.toLowerCase()] || boss.BossName;

    boss?.Supports?.forEach((escort) => {
        escort.BossEscortDifficult = [getDifficulty(config.BossEscortDifficulty, escort.BossEscortDifficult)];
